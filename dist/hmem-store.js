...
export class HmemStore {
                ...
                if (active) {
                    ...
                    if (prefix === "P") {
                        ...
                        for (const o of unassigned) {
                            const existingLinks = o.links ? (() => { try {
                                return JSON.parse(o.links);
                            }
                            catch {
                                return [];
                            } })() : [];
     ...
     */
    resolveProjectO(projectSeq) {
        const oId = `O${String(projectSeq).padStart(4, "0")}`;
        const existing = this.db.prepare("SELECT id FROM memories WHERE id = ?").get(oId);
        if (existing)
            return oId;
        const timestamp = new Date().toISOString();
        ...
        const pExists = pId ? this.db.prepare("SELECT id FROM memories WHERE id = ?").get(pId) : null;
        const links = pExists ? JSON.stringify([pId]) : null;
        this.db.prepare(`INSERT INTO memories (id, prefix, seq, created_at, updated_at, title, level_1, links, min_role)
       VALUES (?, 'O', ?, ?, ?, ?, ?, ?, 'worker')`).run(oId, projectSeq, timestamp, timestamp, title, title, links);
        return oId;
    }
    /**
    ...
    resolveSession(oId, transcriptPath) {
        ...
        if (cached && cached.transcriptPath === transcriptPath) {
            ...
            const nodeExists = this.db.prepare("SELECT id FROM memory_nodes WHERE id = ?").get(cached.sessionId);
            if (nodeExists)
     ...
     */
    resolveBatch(sessionId, oId, batchSize) {
     ...
     */
    getLatestFullBatch(oId, batchSize) {
        return this.db.prepare(`SELECT mn.id, mn.parent_id as sessionId FROM memory_nodes mn
       WHERE mn.root_id = ? AND mn.depth = 3
       AND (SELECT COUNT(*) FROM memory_nodes c WHERE c.parent_id = mn.id AND c.depth = 4) >= ?
       ORDER BY mn.created_at DESC LIMIT 1`).get(oId, batchSize) ?? null;
    }
    /**
     ...
     */
    getPreviousBatch(sessionId, excludeBatchId) {
        return this.db.prepare(`SELECT id, content, title FROM memory_nodes
       WHERE parent_id = ? AND depth = 3 AND id != ? ORDER BY seq DESC LIMIT 1`).get(sessionId, excludeBatchId) ?? null;
    }
    ...
    renameId(oldId, newId) {
        // Validate
        if (oldId === newId)
            return { ok: false, affected: 0, error: "old and new ID are identical" };
        const oldEntry = this.db.prepare("SELECT id, prefix FROM memories WHERE id = ?").get(oldId);
        if (!oldEntry)
            return { ok: false, affected: 0, error: `entry ${oldId} not found` };
        const newExists = this.db.prepare("SELECT id FROM memories WHERE id = ?").get(newId);
        if (newExists)
            return { ok: false, affected: 0, error: `target ID ${newId} already exists` };
        // Ensure same prefix
        const oldPrefix = oldId.match(/^[A-Z]+/)?.[0];
        const newPrefix = newId.match(/^[A-Z]+/)?.[0];
        if (oldPrefix !== newPrefix)
            return { ok: false, affected: 0, error: `prefix mismatch: ${oldPrefix} vs ${newPrefix}` };
        let affected = 0;
        const doRename = this.db.transaction(() => {
            // 1. Rename all child nodes: P0048.1.2 → P0052.1.2
            const nodes = this.db.prepare("SELECT id, parent_id FROM memory_nodes WHERE root_id = ?").all(oldId);
            for (const node of nodes) {
                const newNodeId = node.id.replace(oldId, newId);
                const newParentId = node.parent_id.replace(oldId, newId);
                this.db.prepare("UPDATE memory_nodes SET id = ?, parent_id = ?, root_id = ? WHERE id = ?").run(newNodeId, newParentId, newId, node.id);
                affected++;
            }
            // 2. Rename root entry
            ...
            const tags = this.db.prepare("SELECT entry_id, tag FROM memory_tags WHERE entry_id = ? OR entry_id LIKE ?").all(oldId, `${oldId}.%`);
            for (const t of tags) {
                const newEntryId = t.entry_id === oldId ? newId : t.entry_id.replace(oldId, newId);
                this.db.prepare("DELETE FROM memory_tags WHERE entry_id = ? AND tag = ?").run(t.entry_id, t.tag);
                this.db.prepare("INSERT OR IGNORE INTO memory_tags (entry_id, tag) VALUES (?, ?)").run(newEntryId, t.tag);
            ...
            this.db.prepare("UPDATE hmem_fts_rowid_map SET root_id = ? WHERE root_id = ?").run(newId, oldId);
            const ftsNodes = this.db.prepare("SELECT fts_rowid, node_id FROM hmem_fts_rowid_map WHERE node_id LIKE ?").all(`${oldId}.%`);
            for (const fn of ftsNodes) {
                const newNodeId = fn.node_id.replace(oldId, newId);
                this.db.prepare("UPDATE hmem_fts_rowid_map SET node_id = ? WHERE fts_rowid = ?").run(newNodeId, fn.fts_rowid);
            }
            // 5. Rewrite links in OTHER entries that reference oldId
            const linkRows = this.db.prepare("SELECT id, links FROM memories WHERE links IS NOT NULL AND links LIKE ?").all(`%${oldId}%`);
            for (const lr of linkRows) {
                try {
                    const links = JSON.parse(lr.links);
            ...
            const obsoleteRows = this.db.prepare("SELECT id, level_1 FROM memories WHERE level_1 LIKE ?").all(`%[✓${oldId}]%`);
