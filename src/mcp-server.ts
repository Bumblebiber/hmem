...
server.tool(
    ...
    "To replace the entire tree, use delete_agent_memory + write_memory (curator only).",
  {
    ...
    irrelevant: z.boolean().optional().describe(
      "Mark this root entry as irrelevant [-] (root entries only). " +
      ...
      try {
        ...
        if (useCache) {
          ...
          const promotedIds = new Set(
            entries.filter(e => e.promoted === "favorite" || e.promoted === "access").map(e => e.id)
          );
...
function nodeFav(node: MemoryNode): string {
